...
//3 import { POKEMONS } from '../mock-pokemon-list'; //2 pour pourvoir accès à mes pokémons dans ma liste de pokémons
import { Pokemon } from '../pokemon'; //2 pour avoir accès à ma liste de pokémons 
import { PokemonService } from '../pokemon.service';//3 import pour l'instançation de mon nouveau service 
...
export class DetailPokemonComponent implements OnInit {
  ...
  constructor(
    private route: ActivatedRoute,//1 injection du service ActivatedRoute qui viens de l'import {ActivatedRoute} d'angular/router pour avoir accès à la route courante
    private router: Router, //2 injection du service Router qui va nous permettre d'avoir accès au routeur
    private pokemonService: PokemonService //3 instançation de mon nouveau service 
    ) {} 

  ngOnInit(): void {
    //3 this.pokemonList = POKEMONS; //2 la propriété pokemonList sera un object de type POKEMONS, ça y est on peut accéder à nos pokémons dans notre liste pokémons
    ...
    if (pokemonId) { // on attribut à la propriété pokemonId, le pokemon qui lui correspond
      ...
      //3 this.pokemon = this.pokemonList.find(pokemon => pokemon.id == +pokemonId) //2 pour récupérer le pokemon courant dans la liste de POKEMONS qui correspond au Pokemon demandé par l'URL
    } 
    // else {
